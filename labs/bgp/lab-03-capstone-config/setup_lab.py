#!/usr/bin/env python3
"""
Lab Setup: BGP Lab 03 -- Full Protocol Mastery (Capstone I)

Pushes initial-configs/<node>.cfg (IP addressing only) to each router
via its EVE-NG console port. Console ports are discovered dynamically
from the EVE-NG REST API; no hardcoded port numbers.

PC1 and PC2 read their .vpc files directly from EVE-NG on boot.
...
DEFAULT_LAB_PATH = "bgp/lab-03-capstone-config.unl"
DEVICES = ["R1", "R2", "R3", "R4"]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Push initial configs for BGP lab-03 (capstone I)")
    ...
    print("=" * 60)
    print("Lab Setup: BGP Lab 03 -- Full Protocol Mastery (Capstone I)")
    print("=" * 60)
    ...
    if fail:
        ...
        return 1
    print("[+] Clean-slate IP-only configs applied. Build BGP from scratch per workbook.md.")
    return 0
