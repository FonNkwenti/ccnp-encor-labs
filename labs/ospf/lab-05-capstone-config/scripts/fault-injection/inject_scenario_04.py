#!/usr/bin/env python3
"""
Fault Injection: Scenario 04 -- OSPFv3 Adjacencies Down on Area 0

Target:     R1 (backbone router, Area 0)
Injects:    Removes the `ospfv3 1 ipv6 area 0` statement from R1's
            GigabitEthernet0/0 interface. R1 stops sending OSPFv3 hellos
            on the Area 0 broadcast segment. All OSPFv3 adjacencies from
            R1 drop.
Fault Type: Missing OSPFv3 Interface Enablement
...
PREFLIGHT_SOLUTION_MARKER = "ospfv3 1 ipv6 area 0"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inject Scenario 04 (missing OSPFv3 on R1 Gi0/0)")
