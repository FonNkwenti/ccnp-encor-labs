...
Injects:    IKEv2 keyring peer pre-shared key replaced with an incorrect value.
            IKEv2 Phase 1 AUTH_FAILED — Tunnel0 (GRE-over-IPsec) cannot
            establish an IPsec SA and the line protocol goes DOWN.
Fault Type: IKEv2 Authentication Failure (PSK Mismatch)

Result:     Tunnel0 line protocol goes DOWN on R1.
            `show crypto ikev2 sa` shows no SA or DELETED state.
            `show ip ospf 2 neighbor` shows no R4 (overlay adjacency lost).
            `ping 10.4.4.4 source Loopback0` fails.
            OSPF process 1 (underlay) and VRF CUSTOMER-A paths are unaffected.
