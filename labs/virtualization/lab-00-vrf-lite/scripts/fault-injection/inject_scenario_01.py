#!/usr/bin/env python3
"""
Fault Injection: Scenario 01 -- R3 Transit Path Disruption

Target:     R3 (shared WAN transport router)
Injects:    VRF definition removed from R3 — sub-interfaces lose VRF
            context and all VRF CUSTOMER-A forwarding through R3 drops.
Fault Type: VRF Definition Removal
...
DEVICE_NAME = "R3"
FAULT_COMMANDS = [
    "no vrf definition CUSTOMER-A",
]
PREFLIGHT_CMD = "show running-config | section vrf definition CUSTOMER-A"
PREFLIGHT_EXPECT = "vrf definition CUSTOMER-A"


def preflight(conn) -> bool:
    output = conn.send_command(PREFLIGHT_CMD)
    if PREFLIGHT_EXPECT not in output:
        print(f"[!] Pre-flight failed: R3 does not have '{PREFLIGHT_EXPECT}'.")
