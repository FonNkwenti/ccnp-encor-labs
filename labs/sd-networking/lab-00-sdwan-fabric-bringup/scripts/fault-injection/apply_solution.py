#!/usr/bin/env python3
"""
Solution Restoration: SD-WAN Lab 00 — Fabric Bring-Up

Restores vEdge1 and vSmart to their known-good solution state by pushing
the contents of ../../solutions/{Device}.cfg to each device.

Only vEdge1 and vSmart are affected by the three fault scenarios (Tickets 1-3);
the other devices (vManage, vBond, vEdge2, R-TRANSPORT) are not touched.
...
# (R-TRANSPORT, vManage, vBond, vEdge2 are not modified by any inject script)
VIPTELA_DEVICES = ["vEdge1", "vSmart"]
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Restore SD-WAN lab-00 affected devices to solution state"
    )
    parser.add_argument(
        "--host",
        default="192.168.x.x",
    ...
    print("=" * 60)
    print("Solution Restoration: SD-WAN Lab 00 — Fabric Bring-Up")
    print("=" * 60)
    ...
    print("[*] vEdge1: correct vBond address and organisation-name applied.")
    print("[*] vSmart: VPN 0 default route restored.")
    return 0
