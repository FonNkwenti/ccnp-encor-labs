...
Injects:    EtherChannel mode mismatch on ALL SW3 bundles:
              ...
              Po2 (Gi0/3, Gi1/0): PAgP 'auto' → LACP 'passive'
                  SW1 remains PAgP 'desirable' → LACP vs PAgP protocol
                  mismatch, both sides go down (SD/I).
Fault Type: EtherChannel Mode Mismatch on Po3 (static vs LACP);
            EtherChannel Protocol Mismatch on Po2 (LACP vs PAgP) used
            solely to eliminate the SW1-SW3 bypass path.

Note:       Static 'mode on' was intentionally avoided for Po2 because it
            bundles unconditionally — SW3 would show Po2 SU while SW1
            falls back to stand-alone (I), leaving physical links active
            and bypassing the fault. LACP passive produces a hard protocol
            mismatch that brings both sides to SD.
...
FAULT_COMMANDS_ADD = [
    # Po3: LACP passive — mismatches SW2's static on
    "interface GigabitEthernet0/1", "channel-group 3 mode passive",
    "interface GigabitEthernet0/2", "channel-group 3 mode passive",
    # Po2: LACP passive — protocol mismatch with SW1's PAgP desirable; eliminates bypass path
    # (static 'on' avoided: it bundles unilaterally, leaving SW1 stand-alone and traffic flowing)
    "interface GigabitEthernet0/3", "channel-group 2 mode passive",
    "interface GigabitEthernet1/0", "channel-group 2 mode passive",
]
