# Topology Assets -- Switching Lab 01

This directory holds every artifact a student needs to *reconstruct the topology
in EVE-NG* before running `setup_lab.py`.

Physical wiring is **identical to Lab 00**. This lab bundles the inter-switch
trunks into three EtherChannels (Po1 LACP, Po2 PAgP, Po3 static).
...
| `lab-01-etherchannel.unl` | EVE-NG native lab file | **EVE-NG build** -- import this |

> **If `lab-01-etherchannel.unl` is missing** from this folder, the repo
...
## EtherChannel bundles (configured in this lab)
...
- `.drawio` is **portable and reviewable** -- any contributor (or student)
  can open it without EVE-NG and see the intent: which devices, which links,
  which port-channels.
...
4. Upload `lab-01-etherchannel.unl`
...
`switching/lab-01-etherchannel.unl`. If you import to a different folder,
...
3. Replace `topology/lab-01-etherchannel.unl` in this repo
