#!/usr/bin/env python3
"""
Lab Setup: Multicast Lab 00 -- PIM Sparse Mode, IGMP, and RPF Fundamentals

Pushes initial-configs/<node>.cfg (IP addressing + OSPF) to R1, R2, R3.
PC1 and PC2 read their .vpc files on boot.

OSPF is pre-configured so students can focus entirely on multicast.
PIM-SM, static RP, and IGMP configuration are the student's responsibility.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Push initial configs for Multicast lab-00 (PIM-SM + IGMP)"
    )
    ...
    print("=" * 60)
    print("Lab Setup: Multicast Lab 00 -- PIM Sparse Mode and IGMP")
    print("=" * 60)
    ...
    if fail:
        ...
        return 1
    print("[+] Initial configs applied. Configure PIM-SM per workbook.md Section 5.")
    return 0
