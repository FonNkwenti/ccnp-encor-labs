...
Target:     R1
Injects:    Removes the NETCONF subsystem from R1 so that
            programmatic NETCONF access via port 830 is no longer
            available.
Fault Type: NETCONF disabled
...
PREFLIGHT_CMD    = "show running-config | include netconf-yang"
PREFLIGHT_EXPECT = "netconf-yang"
...
def _require_port(port) -> int:
    if port is None:
        print(
            ...
            "    assigned to R1.  Then update CONSOLE_PORT in this script.",
            file=sys.stderr,
        )
        sys.exit(2)
    return int(port)
...
def main() -> int:
    ...
    parser.add_argument("--port", type=int, default=CONSOLE_PORT,
                        help="Console telnet port for R1 (from EVE-NG UI)")
