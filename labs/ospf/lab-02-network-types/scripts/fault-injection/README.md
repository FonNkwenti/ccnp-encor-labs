# Fault Injection -- OSPF Lab 02 (Network Types + DR/BDR Manipulation)
...
- EVE-NG reachable; lab imported as `ospf/lab-02-network-types.unl`.
...
- `python3 apply_solution.py --host <eve-ng-ip>` has applied the full
  lab-02 solution (priorities + network-type conversion), or the student
  has reached that state manually. Scripts refuse to inject unless the
  device is in the solution state.
