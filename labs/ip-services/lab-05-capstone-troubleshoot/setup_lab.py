#!/usr/bin/env python3
"""
Lab Setup: IP Services Lab 05 -- Comprehensive Troubleshooting Capstone II

Pushes initial-configs/<node>.cfg to R1, R2, R3. These configs are FULLY
configured but contain 6 deliberate faults spanning OSPF, NTP, NAT, and VRRP.
Students must diagnose and fix all faults to restore full IP services operation.

Faults embedded:
  R1: NAT inside/outside reversed, NAT-PAT ACL wrong subnet, VRRP decrement=5
  R2: NTP key-string mismatch, VRRPv3 IPv6 AF missing
  R3: OSPF passive-interface on Gi0/0 (blocks R1-R3 adjacency)
...
def push_device(host: str, name: str, port: int) -> bool:
    ...
    try:
        ...
        conn.save_config()
        print(f"[+] {name}: broken config applied.")
        return True
    except Exception as exc:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Push broken configs for IP Services lab-05 (Capstone II)")
    ...
    print("=" * 60)
    print("Lab Setup: IP Services Lab 05 -- Troubleshooting Capstone II")
    print("=" * 60)
    print("WARNING: Pushing PRE-BROKEN configs. 6 faults are embedded.")
    print("         Use apply_solution.py to restore after completing the lab.")
    ...
    if fail:
        ...
        return 1
    print("[+] Broken configs applied. Diagnose and fix all faults per workbook.md Section 5.")
    return 0
