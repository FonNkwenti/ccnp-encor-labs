#!/usr/bin/env python3
"""
Lab Setup: Multicast Lab 02 -- SSM, Bidirectional PIM, and MSDP

Pushes initial-configs/<node>.cfg to R1, R2, R3, R4.
PC1 and PC2 read their .vpc files on boot.

The initial configs are the Lab 01 end state for R1/R2/R3 (PIM-SM + BSR +
IGMPv3 on receiver LAN) plus a minimal R4 (IP + OSPF only, no multicast).
Students bring R4 into the multicast fabric, fence the two PIM domains with
bsr-border, enable SSM + bidir, then stand up MSDP between R2 and R4.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Push initial configs for Multicast lab-02 (SSM + Bidir + MSDP)"
    )
    ...
    print("=" * 60)
    print("Lab Setup: Multicast Lab 02 -- SSM, Bidir PIM, and MSDP")
    print("=" * 60)
    ...
    if fail:
        ...
        return 1
    print("[+] Initial configs applied. Build SSM/Bidir/MSDP per workbook.md Section 5.")
    return 0
