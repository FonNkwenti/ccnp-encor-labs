# Fault Injection — Security Lab 01
...
- EVE-NG lab `ccnp-encor/security/lab-01-aaa.unl` is running
- All nodes (R1, R2, R3, SW1) are started in the EVE-NG web UI
...
- Each inject script starts from the known-good solution state — run `apply_solution.py` between tickets.
- `apply_solution.py` also generates RSA keys on each device to enable SSH.
