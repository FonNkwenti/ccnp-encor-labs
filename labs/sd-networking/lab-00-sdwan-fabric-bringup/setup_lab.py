...
class LabSetup:
    ...
    def print_viptela_instructions(self):
        ...
        print("=" * 60)
        print()
        print("Viptela OS (vManage, vSmart, vBond, vEdge1, vEdge2)")
        print("does not support automated Netmiko push for initial")
        print("bootstrap configuration.")
        print()
        print("To configure Viptela devices:")
        ...
        print()
        print("Viptela devices to configure manually:")
        viptela_cfgs = [
    ...
    def run(self):
        print(f"SD-WAN Lab 00 — Setup Automation (EVE-NG: {self.eve_ng_host})")
        print()

        # Configure IOS devices via Netmiko
        print("--- IOS Device Configuration ---")
        for name, port, config in self.ios_devices:
            print(f"Setting up {name}...")
            self.push_ios_config(name, port, config)

        # Print manual instructions for Viptela devices
        self.print_viptela_instructions()

        print("Lab Setup Complete.")
        print("After configuring Viptela devices, proceed to workbook.md Task 2.")


if __name__ == "__main__":
    args = parse_args()

    # IOS devices — automated via Netmiko
    # Populate console ports from EVE-NG web UI (Console Access Table in workbook.md Section 3)
    ios_devices = [
