...
class LabSetup:
    ...
    def print_viptela_instructions(self):
        print()
        print("=" * 60)
        print("Viptela SD-WAN Devices — Pre-Broken State")
        print("=" * 60)
        print()
        print("This is a TROUBLESHOOTING capstone lab.")
        print("Each Viptela device is loaded with a PRE-BROKEN configuration.")
        print("Five faults have been injected across the fabric.")
        print()
        print("Apply each Viptela initial-config via console to load the broken state:")
        print()
        ...
        print()
        print("After loading, proceed to workbook.md Section 5.")
        print("Do NOT read initial-configs/ — diagnose using show commands only.")
        print()

    def run(self):
        print(f"SD-WAN Lab 05 — Capstone II Troubleshooting Setup (EVE-NG: {self.eve_ng_host})")
        print()
        ...
        print("The fabric is pre-broken with 5 concurrent faults.")
        print("Use show commands to diagnose. Work bottom-up: control → data → policy.")
