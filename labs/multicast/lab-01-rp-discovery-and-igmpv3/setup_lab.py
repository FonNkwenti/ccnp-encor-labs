#!/usr/bin/env python3
"""
Lab Setup: Multicast Lab 01 -- RP Discovery Mechanisms and IGMPv3

Pushes initial-configs/<node>.cfg to R1, R2, R3.
PC1 and PC2 read their .vpc files on boot.

The initial configs are a verbatim copy of lab-00's end state (PIM-SM + static
RP at 2.2.2.2 + IGMPv2). Students migrate RP discovery from static to Auto-RP
to BSR, then upgrade R3 Gi0/2 to IGMPv3.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Push initial configs for Multicast lab-01 (RP Discovery + IGMPv3)"
    )
    ...
    print("=" * 60)
    print("Lab Setup: Multicast Lab 01 -- RP Discovery and IGMPv3")
    print("=" * 60)
    ...
    if fail:
        ...
        return 1
    print("[+] Initial configs applied. Migrate RP discovery per workbook.md Section 5.")
    return 0
