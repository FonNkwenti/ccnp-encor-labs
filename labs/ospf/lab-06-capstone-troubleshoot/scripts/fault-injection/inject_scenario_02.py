#!/usr/bin/env python3
"""
Fault Injection: Scenario 02 -- No Area 2 Routes in Area 0

Target:     R3 (ABR Area 0/2)
Injects:    Changes the network statement for 10.2.35.0/30 from area 2
            to area 0. Also changes the ospfv3 area assignment on Gi0/1
            from area 2 to area 0. R5 retains area 2 on its side.
            Area ID mismatch prevents adjacency between R3 and R5.
Fault Type: OSPF Area ID Mismatch
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Inject Scenario 02 (area ID mismatch on R3 Gi0/1)")
    ...
    print("=" * 60)
    print("Fault Injection: Scenario 02 (Area ID Mismatch on R3 Gi0/1)")
    print("=" * 60)
