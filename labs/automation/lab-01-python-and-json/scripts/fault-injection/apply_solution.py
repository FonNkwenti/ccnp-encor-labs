#!/usr/bin/env python3
"""
Solution Restoration: Automation Lab 01 — Python Scripting and JSON Construction

Pushes the full solution configs from ../../solutions/ to all devices,
then generates RSA keys to enable SSH and HTTPS. Restores the lab to a
known-good state with RESTCONF enabled on R1 and R2.
...
def restore(host: str, name: str, port: int) -> bool:
    ...
    try:
        ...
        conn.save_config()
        print(f"[+] {name}: solution applied and RSA keys generated.")
        return True
    except Exception as exc:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Restore automation lab-01 to solution state"
    )
    ...
    print("=" * 60)
    print("Solution Restoration: Automation Lab 01 — Python Scripting and JSON")
    print("=" * 60)
    ...
    print("[*] R1 and R2: RESTCONF enabled and HTTPS server active.")
    return 0
